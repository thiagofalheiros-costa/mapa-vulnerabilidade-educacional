...
def test_create_infrastructure_score() -> None:
    ...
    assert result.loc[3, "INFRA_SCORE"] == 0.625


def test_infrastructure_score_is_between_zero_and_one() -> None:
    df = create_test_dataframe()
...
def test_create_public_network_flags() -> None:
    ...
    assert result["IS_FEDERAL"].tolist() == [
        1,
        0,
        0,
        0,
    ...
    assert result["IS_ESTADUAL"].tolist() == [
        0,
        1,
        0,
        0,
    ...
    assert result["IS_MUNICIPAL"].tolist() == [
        0,
        0,
        1,
        0,
    ...
    assert result["IS_PUBLICA"].tolist() == [
        1,
        1,
        1,
        0,
...
def test_create_features_preserves_number_of_rows() -> None:
    ...
    assert "IS_PUBLICA" in result.columns
    assert "IS_FEDERAL" in result.columns
    assert "IS_ESTADUAL" in result.columns
    assert "IS_MUNICIPAL" in result.columns